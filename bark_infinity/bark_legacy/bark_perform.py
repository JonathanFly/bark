    ...
    for file in files:
        ...
        if match:
            ...
            if prefix in lang_dict:
                categorized_files[lang_dict[prefix]].append(
                    FileData(file, name, desc))
            else:
                categorized_files[prefix.capitalize()].append(
                    FileData(file, name, desc))
        else:
...
def print_speakers_list(categorized_files):
    ...
    for category, files in categorized_files.items():
        sorted_files = sorted(files, key=lambda x: (re.search(r"_\w+(_\d+)?\.npz$", x.filename)
                              and re.search(r"_\w+(_\d+)?\.npz$", x.filename).group()[:-4], x.filename))
        print(f"\n  {category}:")
        for file_data in sorted_files:
            name_display = f'  "{file_data.name}"' if file_data.name else ''
            desc_display = f'{file_data.desc}' if file_data.desc else ''
            print(
                f"    {file_data.filename[:-4]} {name_display} {desc_display}")
...
def save_npz_file(filepath, x_semantic_continued, coarse_prompt, fine_prompt, output_dir=None):
    np.savez(filepath, semantic_prompt=x_semantic_continued,
             coarse_prompt=coarse_prompt, fine_prompt=fine_prompt)
...
def split_text(text, split_words=0, split_lines=0):
    if split_words > 0:
        words = text.split()
        chunks = [' '.join(words[i:i + split_words])
...
def gen_and_save_audio(text_prompt, history_prompt=None, text_temp=0.7, waveform_temp=0.7, filename="", output_dir="bark_samples", split_by_words=0, split_by_lines=0, stable_mode=False, confused_travolta_mode=False, iteration=1):
    ...
    for i, chunk in enumerate(text_chunks):
        ...
        print(
            f"Current text chunk ballpark estimate: {estimated_time:.2f} seconds.")
        if longer_than_14_seconds:
            print(
                f"Text Prompt could be too long, might want to try a shorter one or try splitting tighter.")

        audio_array, x = generate_audio(chunk, history_prompt, text_temp=text_temp,
                                        waveform_temp=waveform_temp, base=base, confused_travolta_mode=confused_travolta_mode)
    ...
    if saveit is True:
        save_npz_file(
            f'{filepath}.npz', npzbase[0], npzbase[1], npzbase[2], output_dir=output_dir)

    save_audio_to_file(filepath, concatenated_audio_arr,
...
def main(args):
    ...
    if args.list_speakers:
        ...
        if args.text_prompt:
            ...
            text_prompts_to_process = [
                prompt for prompt in text_prompts_to_process if prompt.strip()]

            print(f"Processing prompts from file: {args.prompt_file}")
            print(
                f"Number of prompts after splitting: {len(text_prompts_to_process)}")

        else:
            print(
                "No text prompt provided. Using the prompts defined in this python file instead.")
        ...
        for idx, prompt in enumerate(text_prompts_to_process, start=1):
            print(
                f"Processing prompt {idx} of {len(text_prompts_to_process)}:")
            ...
            if args.iterations > 1:
                for iteration in range(1, args.iterations + 1):
                    ...
                    gen_and_save_audio(prompt, history_prompt, text_temp, waveform_temp, filename, output_dir,
                                       split_by_words, split_by_lines, stable_mode, confused_travolta_mode, iteration=iteration)
            else:
                gen_and_save_audio(prompt, history_prompt, text_temp, waveform_temp, filename,
                                   output_dir, split_by_words, split_by_lines, stable_mode, confused_travolta_mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--history_prompt", default=None,
                        help="Optional. Choose a speaker from the list of languages: . Use --list_speakers to see all available options.")
    ...
    parser.add_argument("--iterations", type=int, default=1,
                        help="Number of iterations. Default is 1.")
    parser.add_argument("--split_by_words", type=int, default=0,
                        help="Breaks text_prompt into <14 second audio clips every x words")
    parser.add_argument("--split_by_lines", type=int, default=0,
                        help="Breaks text_prompt into <14 second audio clips every x lines")
    parser.add_argument("--stable_mode", action="store_true",
                        help="Choppier and not as natural sounding, but much more stable for very long audio files.")
    parser.add_argument("--confused_travolta_mode", default=False,
                        action="store_true", help="Just for fun. Try it and you'll understand.")

    parser.add_argument(
        "--prompt_file", help="Optional. The path to a file containing the text prompt. Overrides the --text_prompt option if provided.")
    parser.add_argument("--prompt_file_separator",
                        help="Optional. The separator used to split the content of the prompt_file into multiple text prompts.")
