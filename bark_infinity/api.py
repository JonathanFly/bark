...
def determine_output_filename(special_one_off_path=None, **kwargs):
    ...
    text_prompt = re.sub(r' ', '_', text_prompt)
    text_prompt = re.sub(r'[^\w\s]|[:;\'"]', '', text_prompt)
    text_prompt = re.sub(r'[\U00010000-\U0010ffff]', '', text_prompt, flags=re.UNICODE)
    ...
    # Check if file exists and append file numbers
    file_num = 1
    output_filepath = os.path.join(output_dir, base_output_filename)
    base_output_filename_no_ext, file_ext = os.path.splitext(base_output_filename)
    while os.path.exists(output_filepath):
        base_output_filename = f"{base_output_filename_no_ext.rsplit('_', 1)[0]}_{file_num}{file_ext}"
        output_filepath = os.path.join(output_dir, base_output_filename)
        file_num += 1
...
def generate_audio_long(
    ...
    dry_run = kwargs.get('dry_run', None)
    if not dry_run: write_one_segment(audio_arr=np.concatenate(audio_arr_segments),
                                      full_generation=full_generation_segments[0], **kwargs)
...
def render_npz_samples(npz_directory="bark_infinity/assets/prompts/", start_from=None, double_up_history=False,
                       save_npz=False, compression_mode=False, gen_minor_variants=None):
    ...
    for npz_file in npz_files:
        ...
        if gen_minor_variants is None:
            ...
            history_prompt_render_variant = {"semantic_prompt": semantic_tokens, "coarse_prompt": coarse_tokens,
                                             "fine_prompt": fine_tokens}
            ...
            for i in range(gen_minor_variants):
                ...
                coarse_tokens = generate_coarse(semantic_tokens, temp=temp_coarse, top_k=top_k_coarse,
                                                top_p=top_p_coarse, max_coarse_history=max_coarse_history)
...
def chunk_up_text(**kwargs):
    ...
    audio_segments = text_processing.split_general_purpose(text_prompt,
...
def print_chunks_table(chunks: list, left_column_header: str = "Words", right_column_header: str = "Segment Text",
                       **kwargs):
